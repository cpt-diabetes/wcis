...
state_csv = pd.read_csv('data/state_lookup.csv', dtype={2: str}, header=None)
...
def county_lookup(fips):
    """
    given fips code, return county name
    """
    return county_exchange[fips]


def rgb_to_hex(red, green,
               blue):
    """
    Return color as #rrggbb for the given color values.
    Adapted from Jeremy Cantrell's answer on
...
single_colors = ((182, 182, 180), (153, 34, 34))
double_colors = ((6, 93, 149), (182, 182, 180), (153, 34, 34))
...
def sample_color(sample,
    ...
    """
    returns color value of sample, mapped onto range of r_min to r_max,
    corresponding to equivalent location between c0 and c1
    """
    r = (c1[0] - c0[0]) * ((sample - r_min) / (r_max - r_min)) + c0[0]
    g = (c1[1] - c0[1]) * ((sample - r_min) / (r_max - r_min)) + c0[1]
    b = (c1[2] - c0[2]) * ((sample - r_min) / (r_max - r_min)) + c0[2]

    return rgb_to_hex(int(r), int(g), int(b))


def double_color(sample,
                 r_min,
                 r_max,
                 c_low=double_colors[0],
                 c_mid=double_colors[1],
                 c_high=double_colors[2],
                 ):
    # try doing dual_color interpolation
    r_mid = 0.5 * (r_max - r_min) + r_min

    if sample < r_mid:
        return sample_color(sample,
                            r_min=r_min,
                            r_max=r_mid,
                            c0=c_low,
                            c1=c_mid
...
def sample_color_range(
    ...
    """
    If r_array has two elements, we assume that we want the sample color linearly mapped from r_array[0] to r_array[1]
    across the elements of color_array. If r_array has the same number of elements as color_array, the sample color is
    just mapped linearly between whatever interval contains it (note that this means you can get a nonlinear response by
    making r_array and color_array nonlinear to each other). If r_array does not have 2 elements of match color_array
    in length, an error is raised.
    """

    if len(r_array) != len(c_array):
        if len(r_array) != 2:
            raise ValueError(
                f"Problem in sample_color_range function. length of r_array = {len(r_array)}, length of "
                f"c_array = {len(c_array)}. If these lengths do not match, length of r_array must be 2."
            )

    # generate sample range values
    if len(r_array) == 2:
        sample_array = np.linspace(r_array[0], r_array[1], len(c_array))
    else:
        sample_array = r_array

    # make sure colors are in RBG format
    color_array = []
    for c in c_array:
        if type(c) is not tuple:
            if c[0] == "#":
                color_array.append(hex_to_rgb(c))  # assuming this means it's a hex string
            else:  # assuming this is something of the form "rgb(0,0,0)" or "rbga(0,0,0,0)"
                c_ = c.lstrip("rgb")
                c_ = c_.lstrip("a")
                color_array.append(ast.literal_eval(c_))

        else:
            color_array.append(c)

    # find where sample is in sample_array
    bin_bottom_idx = None
    for i, s in enumerate(sample_array[:-1]):
        if (sample >= s) and (sample <= sample_array[i+1]):
            bin_bottom_idx = i
            break

    if bin_bottom_idx is None:
        raise ValueError(
            f"Problem in sample_color_range function. Provided sample is not within defined range from r_array"
        )

    return sample_color(sample,
                        r_min=sample_array[bin_bottom_idx],
                        r_max=sample_array[bin_bottom_idx+1],
                        c0=color_array[bin_bottom_idx],
                        c1=color_array[bin_bottom_idx+1]
                        )
...
def simple_scatter(data,
    ...
    if type(data) != list:
        data = [data]
    if names is None:
        names = [None for _ in range(len(data))]
    if title is None:
        title = ""
    fig = go.Figure()
...
def simple_df_plot(df,
                   ...
                   ylabel_2=None,):
    """
    df: pandas dataframe of data with columns to plot
    x_col: column to use for x data. if not provided, index of dataframe is used
    y_cols: list of or single column name to plot
    ...
    if y_cols is None:
        ...
        y_cols = [y_cols]
    elif (type(y_cols) == float) or (type(y_cols) == int):
        y_cols = [y_cols]
    
    if y_cols_2 is None:
        y_cols_2 = [col for col in df.columns if col != x_col]
    elif type(y_cols_2) == str:
        y_cols_2 = [y_cols_2]
    elif (type(y_cols_2) == float) or (type(y_cols_2) == int):
        y_cols_2 = [y_cols_2]
...
def state_pop(fips=None):
    df_in = pd.read_csv("data/Census/SCPRC-EST2019-18+POP-RES.csv")
    df = df_in.loc[:, ["STATE", "POPESTIMATE2019"]
                   ].query("STATE != 0").rename(columns={"POPESTIMATE2019": "total_pop"})
    df["FIPS"] = df["STATE"].apply(lambda x: str(x) if len(str(x)) == 2 else str(0) + str(x))

    df = df.loc[:, ["FIPS", "total_pop"]]

    # add regions with no pop in the csv, from wikipedia
    extra = pd.DataFrame([['60', '66', '69', '78'], [46366, 168801, 51659, 87146]]).transpose()
    extra.columns = ["FIPS", "total_pop"]
    ...
    if fips is None:
        ...
        return df.query("FIPS==@fips")["total_pop"].squeeze()


def get_state_exchange():
